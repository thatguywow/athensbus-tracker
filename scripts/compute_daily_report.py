...
1. Merge terminus observations from artifacts into DB
2. Reconstruct trips for all routes
3. Compute rotation slots and vehicle activity
4. Compute daily_route_stats rollup
5. Purge raw data older than 30 days
"""
...
def main():
    ...
    with db.job_run("compute_daily_report") as run:
        ...
        try:
            ...
            log.info("Computing daily report for %s across %d routes",
                     service_date, len(route_codes))

            total_trips = total_departures = 0
            errors = []

            # Step 1: reconstruct trips
            ...
            # Step 2: rotation slots + vehicle activity
            log.info("Computing rotation slots...")
            slot_stats = compute_all_slots(conn, service_date, computed_at)
            conn.commit()
            log.info("Slots: %s", slot_stats)

            # Step 3: daily_route_stats rollup
            ...
            run.detail = (
                ...
                f"purged_terminus={purged['terminus_obs']}"
            )
            if errors:
                run.status = "partial"
