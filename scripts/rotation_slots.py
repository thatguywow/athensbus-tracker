...
# ── Πώς μια θέση προγράμματος βρίσκει το δρομολόγιό της: ΔΥΟ ΦΑΣΕΙΣ ───────────
#
# ΦΑΣΗ 1 — ΓΕΜΙΣΜΑ ΚΑΤΑ ΧΡΟΝΟ. ΟΛΑ τα δρομολόγια μπαίνουν στην αντιστοίχιση·
# το πλησιέστερο σε χρόνο γεμίζει την πλησιέστερη θέση, μέσα στα όρια (νωρίς
# ≤9′, αργά ≤40′). Καμία θέση δεν μένει κενή αν υπάρχει δρομολόγιο εντός ορίων
# γι' αυτήν. Αυτό ΗΤΑΝ το αρχικό σχέδιο — ο σχολιασμός της assign_slots το έλεγε
# ρητά: «ALL trips take part in matching».
#
# ΦΑΣΗ 2 — ΑΝΑΒΑΘΜΙΣΗ ΚΑΤΑ ΚΑΛΥΨΗ. Αν ένα αδέσποτο δρομολόγιο έχει ΟΥΣΙΑΣΤΙΚΑ
# πληρέστερη κάλυψη από τον κάτοχο μιας θέσης, και βρίσκεται εντός των ορίων
# της, το ανταλλάσσει· ο κάτοχος γίνεται αδέσποτος. ΠΟΤΕ δεν αδειάζει θέση —
# αλλάζει μόνο ΠΟΙΟΣ την κρατά.
#
# ΓΙΑΤΙ ΕΤΣΙ ΚΑΙ ΟΧΙ ΜΕ ΠΥΛΗ (το λάθος που διορθώνεται εδώ): η «παρατηρημένη
# αναχώρηση» και η «κάλυψη» είναι γεγονότα για τη ΔΙΚΗ ΜΑΣ παρακολούθηση, όχι
# για το αν το λεωφορείο εξυπηρέτησε τη θέση. Ένα καθυστερημένο λεωφορείο που
# το πιάσαμε στα μισά ΕΞΥΠΗΡΕΤΗΣΕ τη θέση· απλώς δεν είδαμε την αρχή. Όταν το
# dep_observed έγινε ΠΥΛΗ αποκλεισμού, 215 θέσεις έμειναν κενές σε μία μέρα
# (2026-08-01) — 209 επειδή το δρομολόγιό τους ήταν dep_observed=False. Η
# κάλυψη ΒΑΘΜΟΛΟΓΕΙ ποιο δρομολόγιο είναι καλύτερο· ΠΟΤΕ δεν αποφασίζει αν ένα
# δρομολόγιο δικαιούται θέση. Εισερχόμενο = εντός ορίων χρόνου, τίποτε άλλο.
#
# ΜΕΤΡΗΜΕΝΟ (2026-08-01), και οι δύο φάσεις μαζί:
#   γραμμή 224 θέση 05:35 — ΠΥΛΗ: κενή· ΦΑΣΕΙΣ: 05:31 (κάλυψη 6%, εκτίμηση) — έτρεξε
#   γραμμή 10  θέση 19:05 — ΠΥΛΗ: 19:06 (απόσπασμα 5%)· ΦΑΣΕΙΣ: 19:25 (πλήρες 98%)
# Η Φάση 1 επαναφέρει 215 κενές θέσεις· η Φάση 2 διορθώνει τα αποσπάσματα που
# κρατούσαν θέση (γραμμή 500, Χ95) χωρίς να αδειάσει καμία.
COVERAGE_SWAP_MARGIN = 0.2   # πόσο πληρέστερη πρέπει να είναι η κάλυψη για ανταλλαγή
...
def align_trips_to_slots(actual_deps: list[float],
    ...
    # ΚΑΘΑΡΟ ΚΟΣΤΟΣ ΧΡΟΝΟΥ — «το πλησιέστερο γεμίζει το πλησιέστερο». Καμία
    # ποινή ποιότητας εδώ· η ποιότητα (κάλυψη) κρίνει ΜΟΝΟ ανταλλαγές στη Φάση 2
    # (upgrade_slots_by_coverage), ποτέ το ίδιο το γέμισμα. Έτσι μια θέση με
    # δρομολόγιο εντός ορίων δεν μένει ποτέ κενή.
    INF = float("inf")
    ...
    dp[0][0] = 0.0
    for j in range(m+1):
        dp[0][j] = 0.0   # leftover scheduled slots are free
    # ΚΑΙ η αντίστροφη περίπτωση: «κατανάλωσα i δρομολόγια χωρίς να χρησιμοποιήσω
    # καμία προγραμματισμένη ώρα». Είναι απολύτως δυνατό — απλώς κοστίζει μία
    # ποινή ανά αταίριαστο δρομολόγιο. Έμενε INF, κι αυτό ήταν σοβαρό σφάλμα:
    # η επιλογή «ταίριασμα» απαιτεί dp[i-1][j-1], άρα για j=1 απαιτεί dp[i-1][0].
    # Με dp[i-1][0]=INF για κάθε i>1, ΜΟΝΟ ΤΟ ΠΡΩΤΟ δρομολόγιο μπορούσε ποτέ να
    # πάρει την ΠΡΩΤΗ προγραμματισμένη ώρα.
    #
    # Το σχήμα: πρώτο δρομολόγιο 04:30, πρώτη προγραμματισμένη ώρα 05:00. Το
    # 04:30 είναι 30′ νωρίς (κόστος άπειρο) και, επειδή ΑΥΤΟ αποτύγχανε,
    # δηλητηρίαζε ό,τι ερχόταν μετά: η 05:00 έμενε κενή ενώ δύο δρομολόγια
    # στέκονταν δίπλα της, το ένα ΑΚΡΙΒΩΣ στην ώρα του (κόστος 0).
    #
    # ΜΕΤΡΗΜΕΝΟ σε ολόκληρο το δίκτυο, ημέρα λειτουργίας 2026-08-01
    # (514 διαδρομές, 11.087 δρομολόγια): +15 ταιριάσματα, 17 διαδρομές
    # άλλαξαν, σε 15 από αυτές ξεκλείδωσε η ΠΡΩΤΗ θέση της ημέρας. Καμία
    # διαδρομή δεν έχασε ταίριασμα και η διάμεση απόκλιση έμεινε 1,8′ —
    # δηλαδή τα νέα ταιριάσματα είναι σωστά, όχι εξαναγκασμένα.
    for i in range(1, n+1):
        dp[i][0] = i * SKIP_ACTUAL_COST
        back[i][0] = ("skip_actual", i-1, 0)
...
def _trip_quality(coverage: float | None, has_arrival: bool) -> float:
    """
    Πόσο ΕΜΠΙΣΤΕΥΟΜΑΣΤΕ ότι αυτό το δρομολόγιο εκπροσωπεί μια πλήρη διαδρομή.
    Κάλυψη (κλάσμα της διαδρομής που είδαμε), με ταβάνι 0,5 όταν λείπει η άφιξη:
    ένα δρομολόγιο χωρίς τέλος δεν είναι ποτέ πλήρης γνώση, όσες στάσεις κι αν
    πιάσαμε. Χρησιμεύει ΜΟΝΟ για ανταλλαγές θέσεων, ποτέ για αποκλεισμό.
    """
    q = coverage or 0.0
    return min(q, 0.5) if not has_arrival else q


def upgrade_slots_by_coverage(assign: list[int | None],
                              actual_mins: list[float],
                              sched_mins: list[float],
                              quals: list[float]) -> list[int | None]:
    """
    ΦΑΣΗ 2 — αναβάθμιση κατά κάλυψη πάνω σε μια ήδη έτοιμη αντιστοίχιση.

    Ένα αδέσποτο δρομολόγιο B παίρνει τη θέση ενός κατόχου A όταν:
      • το B χωράει χρονικά στη θέση (match_cost εντός ορίων),
      • το B έχει ΟΥΣΙΑΣΤΙΚΑ πληρέστερη κάλυψη (quals[B] > quals[A] + περιθώριο),
      • η ανταλλαγή ΔΕΝ σπάει τη χρονική σειρά (το B κάθεται ανάμεσα στους
        κατόχους της προηγούμενης και της επόμενης γεμάτης θέσης).
    Ο A γίνεται αδέσποτος. Καμία θέση δεν αδειάζει· αλλάζει μόνο ο κάτοχος.

    Επαναλαμβάνεται ως το σταθερό σημείο. Κάθε ανταλλαγή αυξάνει το άθροισμα
    κάλυψης-στις-θέσεις κατά >περιθώριο, άρα ο βρόχος τερματίζει.
    """
    assign = list(assign)
    changed = True
    while changed:
        changed = False
        holder = {j: i for i, j in enumerate(assign) if j is not None}
        for b in range(len(actual_mins)):
            if assign[b] is not None:
                continue
            best = None
            for s in range(len(sched_mins)):
                if match_cost(actual_mins[b], sched_mins[s]) == float("inf"):
                    continue
                a = holder.get(s)
                if a is None or quals[b] <= quals[a] + COVERAGE_SWAP_MARGIN:
                    continue
                lo = max((actual_mins[holder[s2]] for s2 in holder if s2 < s),
                         default=float("-inf"))
                hi = min((actual_mins[holder[s2]] for s2 in holder if s2 > s),
                         default=float("inf"))
                if not (lo <= actual_mins[b] <= hi):
                    continue
                gain = quals[b] - quals[a]
                if best is None or gain > best[1]:
                    best = (s, gain, a)
            if best:
                s, _, a = best
                assign[b], assign[a] = s, None
                changed = True
                break
    return assign
...
def assign_slots(conn, route_code: str, service_date: str,
    ...
    # ΟΛΑ τα δρομολόγια μπαίνουν στην αντιστοίχιση (βλ. σχόλιο δύο φάσεων στην
    # κορυφή). Το dep_observed ΒΑΘΜΟΛΟΓΕΙ — δεν αποκλείει: ένα δρομολόγιο που
    # πιάσαμε στα μισά εξυπηρέτησε τη θέση του, απλώς δεν είδαμε την αρχή. Το
    # να το εξορίζαμε άφηνε 215 θέσεις κενές σε μία μέρα.
    actual_mins = [_iso_to_mins_since_midnight(t["started_at"]) for t in trip_rows]

    # ΦΑΣΗ 1 — γέμισμα κατά χρόνο: το πλησιέστερο γεμίζει το πλησιέστερο.
    assignment = align_trips_to_slots(actual_mins, sched_mins)

    # ΦΑΣΗ 2 — αναβάθμιση κατά κάλυψη: ένα αδέσποτο με πληρέστερη κάλυψη και
    # χρόνο εντός ορίων ανταλλάσσει τον κάτοχο μιας θέσης. Καμία θέση δεν
    # αδειάζει· διορθώνει τα αποσπάσματα (γραμμή 500, Χ95) που κρατούσαν θέση.
    quals = [_trip_quality(t["coverage"], t["terminus_arrived_at"] is not None)
             for t in trip_rows]
    assignment = upgrade_slots_by_coverage(assignment, actual_mins,
                                           sched_mins, quals)
