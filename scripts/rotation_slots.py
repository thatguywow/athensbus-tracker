...
# ΜΙΑ ΘΕΣΗ ΕΙΝΑΙ ΜΙΑ ΑΝΑΧΩΡΗΣΗ. Για να τη διεκδικήσεις, πρέπει να έχουμε
# ΠΑΡΑΤΗΡΗΣΕΙ αναχώρηση.
#
# Το match_cost βλέπει ΜΟΝΟ απόσταση χρόνου, ποτέ αν το δρομολόγιο είναι
# αληθινό. Έτσι ένα όχημα που το πιάσαμε μόνο στις τελευταίες στάσεις έπαιρνε
# τη θέση από ένα πλήρες δρομολόγιο λίγο πιο πίσω — και η ώρα αναχώρησής του
# δεν ήταν καν μετρημένη: προκύπτει ΑΝΑΠΟΔΑ από την άφιξη.
#
# ΜΕΤΡΗΜΕΝΟ, γραμμή 500 [ΣΑΒΒ-ΚΥΡ] 2026-08-02, θέση 04:45:
#   04:43  κάλυψη  2%  διάρκεια 1′  στάσεις 87..89  αναχώρηση ΕΚΤΙΜΩΜΕΝΗ  → έπαιρνε τη θέση
#   04:49  κάλυψη 97%  διάρκεια 66′ στάσεις  4..89  αναχώρηση ΠΑΡΑΤΗΡΗΜΕΝΗ → έμενε αδέσποτο
# Το πρώτο δεν ξεκίνησε ποτέ στις 04:43· είναι λεωφορείο που ΤΕΛΕΙΩΝΕΙ
...
# απλώς τα χάσαμε στη πορεία — αληθινή υπηρεσία, δικό μας κενό παρακολούθησης.
#
# ΜΕΤΡΗΜΕΝΟ σε 10.962 θέσεις:
#   όλα διεκδικούν (πριν)          9.811  89,5%   —
#   φραγή σε κάλυψη <60%           9.385  85,6%   11 διαδρομές μηδενίζονται
#   φραγή σε μη παρατηρημένη       9.594  87,5%    4 διαδρομές μηδενίζονται
# Η φραγή στην αναχώρηση κρατά 209 θέσεις παραπάνω ΧΩΡΙΣ να δεχτεί ούτε μία
# εφευρημένη ώρα. Ταυτόσημο αποτέλεσμα με τον σύνθετο κανόνα «κάλυψη ΚΑΙ
# αναχώρηση», οπότε κρατάμε τον απλό.
...
def assign_slots(conn, route_code: str, service_date: str,
    ...
    # ΧΩΡΙΣ ΠΑΡΑΤΗΡΗΜΕΝΗ ΑΝΑΧΩΡΗΣΗ, ΚΑΜΙΑ ΔΙΕΚΔΙΚΗΣΗ ΘΕΣΗΣ. Βγαίνουν εντελώς
    # από την ευθυγράμμιση, ώστε ούτε να πάρουν θέση ούτε να μετατοπίσουν τη
    # σειρά των υπολοίπων. Μένουν χωρίς slot_assignment, άρα η σελίδα τα βλέπει
    # αταίριαστα· όσα είναι και αποσπασματικά τα κρύβει το φίλτρο κάλυψης.
    #
    # Το δρομολόγιο ΔΕΝ χάνεται: εξακολουθεί να υπάρχει, με τη διαδρομή και τις
    # ώρες που όντως μετρήσαμε. Αυτό που δεν κάνει πια είναι να δηλώνει ότι
    # εξυπηρέτησε μια συγκεκριμένη προγραμματισμένη αναχώρηση.
    trips_for_slots = [t for t in trip_rows if t["dep_observed"]]
    if not trips_for_slots:
        return {"assigned": 0, "handoffs": 0}
    ...
    for trip, sched_idx, amins in zip(trips_for_slots, assignment, actual_mins):
